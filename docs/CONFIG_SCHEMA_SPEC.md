...
```
public/configs/
  prod/<id>.json       ← One questionnaire or battery per file, filename = entity
                         id = config id (e.g. phq9.json, trauma_eval.json).
                         Dev/test fixtures live here too, marked "dev": true at
                         the config top level (never shown in the production
                         composer).
```

There is no manifest — the composer's catalog is generated by scanning these directories (`npm run build:catalog`). The `validate:configs` script checks schema validity, cross-file ID uniqueness, and the one-entity-per-file / filename=id layout rule.
