...
Config files are language-specific. Text fields are plain strings — the language is implicit in which file is loaded. There is no localisation nesting: a translation is a sibling file under `prod/<lang>/` with the same id and structure (see "Config file layout" and `docs/I18N_SPEC.md`).
...
```
public/configs/
  prod/<id>.json        ← Hebrew (canonical). One questionnaire or battery per
                          file, filename = entity id = config id (e.g. phq9.json,
                          trauma_eval.json). Dev/test fixtures live here too,
                          marked "dev": true at the config top level (never
                          shown in the production composer).
  prod/<lang>/<id>.json ← A translation of prod/<id>.json (e.g. prod/en/phq9.json):
                          same id, same structure, only text differs. <lang> is a
                          code from shared/i18n/core.js LANGS (never "he").
```

There is no manifest — the composer's catalog is generated by scanning these directories (`npm run build:catalog`). The `validate:configs` script checks schema validity, cross-file ID uniqueness (per language), the one-entity-per-file / filename=id layout rule, and **translation parity**: a `prod/<lang>/<id>.json` file must deep-equal `prod/<id>.json` after every text field is blanked (`title`, `description`, `keywords`, `text`, `label`, `message`, `ratingText`, the values of `subscaleLabels` and slider `labels`, and `meta.source` / `psychometrics.source`), its `dependencies` must be the Hebrew list rewritten into `prod/<lang>/`, and every translated questionnaire must carry `meta.source`. A translated battery is only offered in that language when every questionnaire it sequences is translated too (`shared/config/translation-parity.js`, `docs/I18N_SPEC.md` §4).

The patient app selects the directory from the link's `lang=` parameter (absent ⇒ Hebrew) via `configBaseFor(lang)`; ids are language-independent, so trajectories in the Aggregate line up across languages.
...
| `source` | string | translations: yes · Hebrew: recommended | Provenance of this file's text: the publication, official translation, or `"Madad original"` it was transcribed from. `validate:configs` requires it on every `prod/<lang>/` questionnaire. |
