...
**Currently working on:** idle. Multi-language phase 1 (`I18N-1..7`, Hebrew + English) landed on branch `i18n` on 2026-09-17 (archive A-17, decision D-19) and **reached `main` on 2026-09-18** via `composer-ui`, which is now merged and deleted along with `i18n`. The English instrument set went from 9 to 40 the same day (`I18N-10`), plus SAPAS in both languages. Before that: `AGG-7` landed 2026-08-22 (archive A-15) and `AGG-9` was filed the same day (its capture side landed with I18N-1). The `CONT` band is cleared — CONT-1/2/3 all landed 2026-08-21 (archive A-12/A-13/A-14, decision D-18). Before that, the same day, a documentation pass refreshed `HANDOVER.md` and the specs against the tree. If the idiographic stream is picked up next, `IDIO-0` is the decision gate everything else waits behind.
...
Phase 1 (Hebrew + English) landed on branch `i18n`, 2026-09-17 (archive A-17, decision D-19). Later phases:

| ID | Title | Status | Notes |
|---|---|---|---|
| I18N-1..7 | Foundation · patient app + PDF · English starter set · Composer + nav · Aggregate · Help + Landing · docs | done | See A-17. |
...
| I18N-10 | More English instruments | in progress | 40 of 54 done (2026-09-18/19). Outstanding, each needing a source: `isi` (the file supplied was a compendium chapter, no items), `pqb` (the PQ-16 was supplied; the config is the 21-item PQ-B — different instrument), `cape42`. Hebrew-origin, so they need translation rather than transcription: `anger_log`, `ocsrs_m`, `novaco_anger_situations`. Method: `node scripts/scaffold-translation.mjs en <id>`, transcribe, cite in `meta.source`, `npm run validate:configs && npm run build:catalog`. |
| I18N-11 | `validate:configs` should reject unfilled scaffolds | todo | `scripts/scaffold-translation.mjs` writes `"TODO: <hebrew>"` placeholders, and the validator passes a file that still contains them — hit on `clinical_intake` 2026-09-18, caught only by grepping. A one-line check closes it. |
...
| IDIO-3 | Composer: slots dialog + cart row states + pid-keyed param store | todo | Blocked on IDIO-2. Plan §3. **Cheaper since D-20:** the rail and the sheet now share `<session-settings>` and one list renderer, so the "both files and both test files" cost is largely paid. The `.c-chip` idiom (dashed = empty slot) is what a per-instrument param summary should reuse. Params must be keyed by pid — otherwise patient B gets patient A's problem descriptions. |
...
### D-20 — The composer rail holds everything: actions on top, settings as value chips
**Date:** 2026-09-17
**Context:** The 300px rail carried three stacked sections (link + QR + actions, patient ID, selected list), each with its own label and hint sentence, and read output-first — the instruments being picked sat at the bottom. Merging `i18n` added a fourth section (patient language + a dismissible "N dropped" notice), and `IDIO-3` is queued to add per-instrument slots to the same rows. Two structural facts forced the timing: `selection-cart.js` and `mobile-bar.js` were the only two composer files that conflict in the i18n merge (i18n branched before the QR commit `02d2f4c`, so the QR row had to be re-applied by hand anyway), and they were two renderers of one output model — which is exactly what `IDIO-3` is priced on.
**Decision:** (1) The rail keeps everything, as **one fixed head over one scrolling list**: the link and its actions lead, the settings chips sit beneath them, and the picked list is the only part that scrolls — so nothing above it can ever be displaced. Actions lead because the rail's payoff is the link; the chips follow because they are attributes *of* that link (language and ID change what it contains), so they read as its metadata rather than as a separate concern. (2) **One renderer, two hosts** — `<selection-cart>` is the rail on desktop and the body of the phone's sheet (`compact`). This is what kills the duplication. (3) Head order: **URL · primary · icon row · chips**. The URL stays, above the buttons — muted monospace clamped to *exactly* two lines (not a minimum: the placeholder wraps to one where a real URL takes two, which shifted the block by 3px). It is a receipt; a glance confirms the link is what the clinician expects before it reaches a patient. The primary spans the full width with a row of equal secondary buttons (QR · ↗ · share) beneath it — a single row cannot hold a long Hebrew label plus three buttons in 260px usable, and shrinking the primary would blunt the one control that must be unmistakable. (4) Settings present as **value chips**: the closed state is the settings themselves, and an unset chip is dashed — an empty slot, not a disabled control. (5) The **language chip is a `<select>`** wearing a chip: one click reaches the native list. Not a toggle — the language list is open-ended (I18N-8/9). The nav's UI-language switch is a select for the same reason. (6) The **ID chip reveals one labelled field and focuses it**, so it is one click to start typing while keeping the label and the "PDF only" hint. It opens itself on an invalid pid or on entries dropped by a language switch, and never forces itself closed. (7) The QR tile becomes an icon button onto the dialog `<qr-code>` already owns (new `tileless` mode + public `expand()`), removing the ~112px jump the rail made on the first pick. The head is now measurably immobile: 264px at y=73 whether nothing is picked, three are, or the pid field is open. (8) **Phones keep a bar**, because they have no rail: a chevron'd count button that opens the sheet, QR, and the primary action. No URL at that width — it always clipped, which is also what satisfies the 320px reflow check. (9) Reorder arrows in the rail reveal on hover/`:focus-within` via `opacity`, never `display:none`; the sheet keeps them visible. (10) **Catalog card pins are unchanged** — the browse column was left alone.
**Rejected alternatives:** **a bottom action bar shared by desktop and phone** — built first, then reverted: bottom-anchored actions are a phone idiom, and with a rail already on screen they put the link where the eye does not go ("the action buttons don't pop out, I need to search for them"). It was only ever a means to collapse the two renderers, and decision 2 achieves that without it. **Output pinned at the foot of the rail** — built next, also reverted: still the last thing the eye reaches. **Settings above the actions** (reads in working order, but two small chips become the first thing the eye lands on, which is not what the rail is for) and **a one-row action block** (buys back ~52px of list, but wraps as soon as the platform offers Web Share, and shrinks the primary). Also rejected: trimming the rail in place (leaves two renderers, so IDIO-3 still pays twice); a labelled "הגדרות ▾" button or a label/value session card instead of chips (both say something *about* the settings rather than showing them, and neither grows into the idiographic param slots); an ID chip that becomes the input in place (one click, but no room for the label or the format hint); segmented language toggles (do not survive a third language); hover-revealing the catalog pins (rejected by the user — the browse column stays as it is).
**Defects found and fixed in review:** the settings drawer shipped permanently open — `?hidden` does not hide an element whose own CSS sets `display: flex`, and the unit test asserted the attribute rather than the absence, so it passed; `compact` on `<selection-cart>` drives `:host([compact])` rules but was not reflected, so none of them applied; the output block still grew 3px on the first pick because the placeholder wrapped to one line where a real URL takes two (fixed with an exact two-line `block-size`); the URL line failed WCAG AA at 3.41:1 until it used `--clin-rail-hint`, the token the earlier a11y pass tuned for this navy; `npm run lint` failed on `i18n` itself because its rewritten `help/src/help.js` uses `document` while `help/src` was never added to eslint's browser-globals block.
**Scope impact:** New `session-settings.js` (+ test). `selection-cart.js` rewritten as the two-region panel (fixed head, scrolling list); `mobile-bar.js` rewritten as bar + sheet around that panel; `composer-app.js` layout. `.c-chip` added to `clinician-styles.js`; `qr-code.js` gains `tileless` + `expand()` and was translated (it landed on `main` after `i18n` branched, so it had no string keys); `clinician-nav.js` language pill → select. Composer chunk 27.9 KB gz of a 30 KB budget. The e2e suite's per-viewport skips are gone: selection, PID, reset and launch now run in both viewports against the same component.
