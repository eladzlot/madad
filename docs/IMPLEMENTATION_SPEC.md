...
- **Components:** Lit (web components). Used for all UI components; logic modules (`src/engine/`, `shared/`) remain pure JS with no framework dependency.
...
│   │   ├── item-multiselect.js
│   │   └── progress-bar.js
│   ├── helpers/gestures.js           # Touch/pointer gesture utilities (swipe, overscroll)
│   ├── pdf/report.js                 # PDF generation
│   └── styles/
│       ├── main.css
│       └── reset.js
├── shared/                           # Pure logic shared by every surface
│   ├── config/
│   │   ├── loader.js                 # Fetches, validates, merges config
│   │   ├── config-validation.js      # Post-AJV semantic validation
│   │   ├── item-types.js             # Item type registry — single source of truth for type metadata
│   │   ├── options.js                # Option-set resolution
│   │   ├── QuestionnaireSet.schema.json
│   │   └── validate-schema.js        # GENERATED by npm run build:validator
│   ├── catalog/build-catalog.js      # Builds the composer catalog index
│   ├── pdf/envelope-schema.js        # Embedded data.json envelope + validateEnvelope
│   ├── pid.js                        # PID pattern + helpers
│   └── styles/tokens.css             # Design tokens incl. dark mode
├── clinician/                        # Shared clinician shell (nav + design vocabulary)
├── composer/
│   ├── index.html
│   └── src/
│       ├── composer.js               # Composition root
│       ├── composer-store.js         # Reactive store (single source of truth)
│       ├── composer-loader.js        # Fetches generated catalog index
│       ├── composer-state.js         # URL builder, pid handling, getAppRoot()
...
│   └── e2e/                          # Playwright specs (incl. *.dist.test.js)
├── scripts/
...
**No DOM in logic modules.** Everything under `src/engine/` and `shared/` must be pure logic with no DOM access. This makes unit testing trivial and keeps concerns separated. DOM manipulation lives only in `src/components/` (Lit web components), `src/controller.js`, and `src/app.js`.
...
- An optional `dependencies` array — relative paths to other config files that must be loaded alongside this one. `loadConfig` auto-fetches them patient-side (BFS walk), so generated URLs name only the selected items. Example: `clinical_intake.json`, whose battery references the DIAMOND screener and seven questionnaires, declares `"dependencies": ["configs/prod/diamond_sr.json", "configs/prod/phq9.json", ...]`.
...
An interpretations block also carries an optional `type` (`severity` | `screening`) and an optional `cutoffs[]` array of explicit thresholds. `severity` ranges render as background bands in the Aggregate's trajectory charts; `cutoffs` render as solid lines. Absent `type` means no chart overlay is derived from the ranges.
...
- A message string displayed in the PDF (badge label, max 30 characters)
- A required `severity`: `critical` (red pill — safety risk) or `warning` (amber pill — clinical attention). These are the only two values; an earlier `info` level was removed from the schema. Alerts sort critical-first in the PDF.
...
### 8.2 Cross-file references

A battery may sequence questionnaires defined in other files. When it does, declare the dependency explicitly:
...
The patient app's `loadConfig` auto-fetches declared dependencies at runtime (BFS walk, `loadDependencies: true` by default). Battery files must declare a dependency on every questionnaire file their sequence references — `validate:configs` errors on undeclared cross-file references. All IDs must remain globally unique across all loaded files; the loader enforces this at runtime.

Generated patient URLs name only the items the clinician selected — dependency resolution happens patient-side, so a battery URL stays short.
...
The `validate:configs` script runs over every file in `public/configs/prod/` — dev fixtures must still be schema-valid and follow the one-entity-per-file rule.
...
|---|---|---|
| Short name (alphanumeric, `_`, `-` — no slashes, no `.json`) | `phq9` | → `/configs/prod/phq9.json` (root-relative) |
| Relative path with slash or `.json` | `configs/prod/phq9.json` | → `/configs/prod/phq9.json` (prepended to root-relative) |
| Root-relative path | `/configs/prod/phq9.json` | used as-is (absolute from origin) |
...
### 9.5 No default config

There is no default config. Config sources are derived entirely from the `items=` tokens (item IDs are addresses — §8.1), so a URL without `items` has nothing to load and shows a pre-welcome error screen. The legacy `configs=` parameter is read by nobody: `src/app.js` ignores it.
...
|---|---|---|---|
| `items` | Yes | — | Comma-separated ordered list of questionnaire or battery IDs. Each token is also its config address — see §8.1 and §9.1. |
| `#pid` | No | — | Optional patient identifier, read from the URL fragment first (`#pid=…`, what the composer generates) and from `?pid=` as a legacy fallback. Validation rules in `shared/pid.js`. |
| `configs` | — | — | Legacy bundle-era parameter. Parsed by nothing; deliberately ignored. |

If `items` is absent or empty, a pre-welcome error screen is shown in Hebrew.
...
- **URL parameter validation.** `pid` is validated against `PID_PATTERN` in `shared/pid.js` (ASCII + Hebrew letters, digits, `-`, `_`, length 1-64) and silently dropped if invalid. `items` is parsed as a comma-separated list and each token is validated by `loadConfig`/`resolveItems`. Invalid tokens produce an error screen; they are never reflected back into the UI.
...
- `configs/prod/phq9.json` from `https://app.ezmadad.com/` → `https://app.ezmadad.com/configs/prod/phq9.json` ✓
- `configs/prod/phq9.json` from `http://localhost:5173/` → `http://localhost:5173/configs/prod/phq9.json` ✓
