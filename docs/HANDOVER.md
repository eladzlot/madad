...
| Path-traversal rejection (`..`, root-relative included), backslash rejection, `//` authority rejection, + short-name allowlist (`SHORT_NAME_RE`) | `shared/config/loader.js` | Crafted `items=` tokens escaping the config base, or reaching a foreign origin without passing `allowedOrigins`. `//evil.com/c.json` and `/\evil.com/c.json` both resolve cross-origin but are not `https?://` URLs, so they must be rejected before the root-relative branch. |
| Item-token allowlist (`ITEM_TOKEN_RE`) on `?items=` | `src/app.js` | The one config source that comes from a stranger. Every other `loadConfig` call site passes catalog- or envelope-derived IDs; this one is a URL anyone can edit, so tokens are held to short names at the boundary before they become fetch targets. |
...
| Name length cap (200 chars) + BiDi strip | `shared/text-hygiene.js`, applied in `src/components/welcome-screen.js` | Oversized or directionally-manipulated names in the PDF. |
| Inbound `pid`/`name` cap + BiDi strip (`sanitizeEnvelope`) | `shared/pdf/envelope-schema.js`, applied at the parse boundary in `aggregate/src/parse-pdf.js` | The write-path caps above bind only PDFs we generated; an uploaded PDF's attachment can be hand-built. Aggregate renders these fields, including into an exported SVG whose `esc()` escapes markup but not BiDi controls. Sanitizes rather than rejects — the pid groups a patient's sessions, so dropping one would silently split a trajectory. |
| ReDoS guard (`isSafePattern` + input length cap) | `shared/safe-pattern.js`, applied in `src/components/item-text.js` and `shared/config/config-validation.js` | Config-supplied `pattern` fields causing catastrophic regex backtracking. Rejects nested quantifiers, adjacent quantified groups, *and* quantified alternation (`(a|a)+` — 78s against 30 characters). JS cannot abort a running match, so the shape is refused before it runs and the input it runs against is bounded. Config validation rejects such patterns in CI so an author learns at commit time rather than having validation silently skipped at runtime. |
...
- **Security controls in §6a** — specifically: do not revert error rendering to `innerHTML`, do not remove PID/name validation, do not remove the `allowedOrigins` check from `loadConfig`, and do not reorder the source checks in `resolveSource` so the root-relative branch runs before the `..`/backslash/`//` rejections. Each was introduced to fix a concrete vulnerability.
