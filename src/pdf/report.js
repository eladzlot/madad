...
// ── Branding ──────────────────────────────────────────────────────────────────
const getAppUrl = () => {
  if (typeof window === 'undefined') return 'https://ctrmadad.com/';   // remote deployment (non-browser fallback only)
...
export function buildDocDefinition(sessionState, config, session, now = new Date(), { lang = currentLang(), timing = null } = {}) {
  ...
  const envelope = buildEnvelope({ sessionState, config, session, appVersion: APP_VERSION, lang, now, timing });
