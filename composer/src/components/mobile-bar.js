...
export class MobileBar extends LitElement {
  ...
  // The trial's uid is mandatory, so while it is missing the bar's primary has
  // nothing useful to do: it says what is missing and lands the cursor there.
  async _promptForPid() {
    this._open = true;
    await this.updateComplete;
    const panel = this.renderRoot.querySelector('.sheet selection-cart');
    await panel?.updateComplete;
    await panel?.focusPid();
  }
  ...
  render() {
    ...
    return html`
      <div class="bar">
        <!-- Never disabled on the trial: the uid is entered in the sheet, and
             it has to be reachable before anything is picked. -->
        <button class="c-btn count-btn" type="button"
          aria-expanded=${this._open ? 'true' : 'false'}
          ?disabled=${!count && !this.requirePid} @click=${this._openSheet}>
