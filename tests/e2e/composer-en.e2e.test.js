...
const langBtn = (page, code) => page.locator(`selection-cart .lang-seg button[lang="${code}"]`);

test.describe('composer in English', () => {
  test.beforeEach(async ({ isMobile }) => {
    test.skip(isMobile, 'the cart and its language switch are desktop-only');
  });
  ...
  test('?lang=en renders an LTR English UI with translated titles', async ({ page }) => {
    ...
    await expect(page.locator('clinician-nav .lang button[lang="en"]')).toHaveAttribute('aria-pressed', 'true');
    ...
    await expect(langBtn(page, 'en')).toHaveAttribute('aria-pressed', 'true');
  ...
  test('the nav toggle reloads into the other language and remembers it', async ({ page }) => {
    await page.goto('/composer/?lang=en');
    await expect(brand(page)).toBeVisible({ timeout: 10_000 });
    await page.locator('clinician-nav .lang button[lang="he"]').click();
  ...
  test('a Hebrew clinician can send an English link; the switch drops Hebrew-only picks', async ({ page }) => {
    ...
    await langBtn(page, 'en').click();
    await expect(urlBox(page)).toContainText('items=phq9');
    await expect(urlBox(page)).toContainText('lang=en');
    await expect(page.locator('selection-cart .dropped')).toContainText('English');
    await expect(card(page, 'wsas')).toHaveCount(0);
