...
test.describe('dist smoke — production bundle at production base', () => {
  ...
  test('composer page loads the catalog without 404s', async ({ page, baseURL }) => {
    const origin = new URL(baseURL).origin;
    const badResponses = watchForBadResponses(page, origin);
    const consoleErrors = watchForConsoleErrors(page);
    ...
    // The composer bootstraps by fetching catalog.json (relative to
    // /composer/, generated by scripts/build-catalog.mjs) — a code path
    // entirely separate from the patient app's loadConfig short-name
    // expansion. A path bug could live here independently of the one that
    // produced "לא ניתן לטעון" in the patient app, so this test must do more
    ...
    // when catalog loading throws (see composer.js). Assert it's absent —
  ...
  test('patient page resolves cross-config dependencies from a minimal composer URL', async ({ page, baseURL }) => {
    ...
    const realErrors = consoleErrors.filter(e => !/favicon/i.test(e));
    expect(realErrors, 'No CSP violations or runtime errors on dependency auto-fetch').toEqual([]);
  });
});
