...
// Trial: the pid is a minted uid and it is mandatory (REMOTE_SPEC §3), so
// every patient link in this suite carries a valid one.
const UID = 'E2E0-0017';
const EN_URL = `/?items=phq9&lang=en#pid=${UID}`;
...
test.describe('English patient flow', () => {
  ...
  test('an unknown language is a malformed link, not a silent Hebrew fallback', async ({ page }) => {
    await page.goto(`/?items=phq9&lang=xx#pid=${UID}`);
  ...
  test('an instrument without an English file fails to load rather than showing Hebrew', async ({ page }) => {
    ...
    await page.goto(`/?items=ocsrs_m&lang=en#pid=${UID}`);
