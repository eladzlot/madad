...
test.describe('aggregate round-trip', () => {
  ...
  test('two sessions chart as two markers with a connecting line (no dedup)', async ({ page }) => {
    ...
    // an alert ring, and that ring is DOUBLED (it separates from the series by
    // lightness rather than hue, so the second ring is what keeps it reading as
    // an alert rather than as the focus state): 2 markers + 4 rings.
